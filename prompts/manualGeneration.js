...
- 代表的な手順ごとに画面左下のタイムスタンプを参照し、そのファイル名（例: 00:00:01.png）で画像をマークダウン内に**必ず埋め込んでください**（\`![](./00:00:01.png)\`）。画像は必ず表示されるように記載してください。
...
2. 各選定した画像（タイムスタンプ付きファイル名）ごとに以下を行います：
   - 画像を**必ず**マークダウンで埋め込み（例: \`![](./00:00:01.png)\` ※必ずこの形式で明記）
...
- マークダウン形式のドキュメント
- 各作業ステップごとに画像（例: \`![](./00:00:01.png)\`）、その説明、使用材料・工具、手順・注意点を記載
- 文章は初心者向けに分かりやすく簡潔に
...
【要約】  
画像ごとにタイムスタンプ付きファイル名を使い、手順・材料・工具・理由を推理して初心者向けに分かりやすく解説したマークダウン資料を作成してください。とくに画像のマークダウン埋め込み部分（例: \`![](./00:00:01.png)\`）は強調して「必ず」記載し、出力に反映されるように気を付けてください。`;
